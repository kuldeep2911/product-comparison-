...
def ai_search_flow():
    ...
    while True:
        query = input("\nEnter your search query (e.g. 'best gaming phone under 50000 with big battery' or 'iPhone vs Samsung camera') or 'q' to go back: ").strip()
        if query.lower() == 'q':
            break
        if not query:
            continue
        ...
        try:
            ...
            try:
                ...
                explanation = generate_explanations(
                    top_results,
                    ranked_products,      # передаём полный список для diversity внутри generate_explanations
                    query,
                    structured_query.brands
                )
